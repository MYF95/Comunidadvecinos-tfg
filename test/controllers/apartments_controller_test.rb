...
class ApartmentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'update apartment should work properly as admin user with correct data' do
    log_in_as(@admin)
    get edit_apartment_path(@apartment)
    patch apartment_path(@apartment), params: { apartment: { owner: full_name(@admin), floor: 1, letter: 'B', fee: 60}}
    assert_not flash.empty?
  ...
  #TODO mejorar el test "Update apartment should not work as admin user with incorrect data"
  ...
  #TODO integration test for view apartment?
